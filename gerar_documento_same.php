<?php 
    ...
    if(!isset( $row_aten['CD_PACIENTE']) && isset($_GET['cd_paciente'])){
        $_SESSION['msgerro'] = "Número de prontuario não encontrado."; 
    }
    ...
    @$var_cd_atendimento = 416028;


    ///////////////////////////////////////////
	//Verifica se existe prontuario (editor)///
    ///////////////////////////////////////////
	
	if(isset($_GET['cd_paciente']) OR isset($_SESSION['atdpdf']) ){
        $cons_pdf ="SELECT 
                        CASE 
                            WHEN COUNT(CD_PACIENTE) > 0 THEN COUNT(CD_PACIENTE)
                            ELSE NULL
                        END AS doc
        ...
        @$var_pdf_existe = $row_pdf_exis['CD_PACIENTE'];
	...
	if(isset($_GET['cd_atendimento']) OR isset($_SESSION['atdpdf']) ){
        ...
        $result_pdf_exiss = oci_parse($conn_ora, $cons_pdff);
        @oci_execute($result_pdf_exiss);
        @$row_pdf_exiss = oci_fetch_array($result_pdf_exiss);
        @$var_pdf_existee = $row_pdf_exiss['BLOB_ANEXO'];
        }
...
<body>
	...
	<div class="div_br"> </div>
        ...
        <?php if(strlen(@$var_nm_paciente) > 1 ){ ?>
            ...
            <form autocomplete="off" id="assinatura"  method="get//" action="gerar_documento_pdf_guia_same.php">
                <div class="row">		
                    ...
                    <div class="col-md-0" id="div_sn_exame_mv">
                            ...
                            <input type="hidden"  class="form-control" value="<?php echo @$var_cd_paciente?>" id="prontuario" name="cd_paciente" readonly></input>
                    </div>
...
$(document).ready(function(){
    ...
    $(document).on('shown.bs.modal','.modal', function (event) {
        ...
        if(identificador == 'same'){
            ...
            $.getJSON('visualizar_guia_same.php?search=',{cd_atendimento: cd_atendimento, cd_paciente: cd_paciente,nm_paciente: nm_paciente, ajax: 'true'}, function(j){
    ...
    document.getElementById("sig-submitBtn").addEventListener("click", function () {
        ...
        var cd_paciente = document.getElementById("prontuario").value;
		...
		console.log(dt_aten);
            ...
            $.ajax({
                ...
                data: {cd_atendimento: cd_atendimento, cd_paciente: cd_paciente,nm_paciente: nm_paciente, escondidinho:escondidinho},//Dados para consulta
        ...
        //$.getJSON('gerar_documento_pdf.php?search=',{cd_paciente: cd_paciente,nm_paciente: nm_paciente,dt_aten: dt_aten,nm_conv: nm_conv,escondidinho:escondidinho, ajax: 'true'}, function(j){

        //});

        //$.getJSON('gerar_documento_pdf_contrato.php?search=',{cd_paciente: cd_paciente,nm_paciente: nm_paciente,dt_aten: dt_aten,nm_conv: nm_conv,escondidinho:escondidinho, ajax: 'true'}, function(j){
